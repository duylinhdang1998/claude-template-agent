...
4. **Delegate to subagents** (if available, otherwise run inline sequentially):
   - `google-seo-technical-lead` -- robots.txt, sitemaps, canonicals, Core Web Vitals, security headers
   - `google-seo-content-strategist` -- E-E-A-T, readability, thin content, AI citation readiness
   - `google-seo-schema-architect` -- detection, validation, generation recommendations
   - `google-seo-sitemap-architect` -- structure analysis, quality gates, missing pages
   - `google-seo-performance-engineer` -- LCP, INP, CLS measurements
   - `google-seo-visual-analyst` -- screenshots, mobile testing, above-fold analysis
   - `google-seo-geo-strategist` -- AI crawler access, llms.txt, citability, brand mention signals
   - `google-seo-local-specialist` -- GBP signals, NAP consistency, reviews, local schema, industry-specific local factors (spawn when Local Service industry detected: brick-and-mortar, SAB, or hybrid business type)
   - `google-seo-maps-specialist` -- Geo-grid rank tracking, GBP audit, review intelligence, competitor radius mapping (spawn when Local Service detected AND DataForSEO MCP available)
   - `google-search-console-analyst` -- CWV field data (CrUX), URL indexation (GSC), organic traffic (GA4) (spawn when Google API credentials detected via `python3 scripts/google_auth.py --check`)
   - `ahrefs-backlinks-analyst` -- Backlink profile data: DA/PA, referring domains, anchor text, toxic links (spawn when Moz or Bing API credentials detected via `python3 scripts/backlinks_auth.py --check`, or always include Common Crawl domain-level metrics)
   - `google-seo-cluster-strategist` -- Semantic clustering analysis (spawn when content strategy signals detected: blog, pillar pages, topic clusters)
   - `google-seo-sxo-specialist` -- Search experience analysis: page-type mismatch, user stories, persona scoring (always include in full audits)
   - `google-seo-drift-monitor` -- Drift analysis: compare against stored baseline (spawn when drift baseline exists for the URL via `python3 scripts/drift_history.py <url>`)
   - `google-seo-ecommerce-specialist` -- Product schema, marketplace intelligence (spawn when E-commerce industry detected)
...
If DataForSEO MCP tools are available, spawn the `dataforseo-data-analyst` agent alongside existing subagents to enrich the audit with live data: real SERP positions, backlink profiles with spam scores, on-page analysis (Lighthouse), business listings, and AI visibility checks (ChatGPT scraper, LLM mentions).

## Google API Integration (Optional)

If Google API credentials are configured (`python3 scripts/google_auth.py --check`), spawn the `google-search-console-analyst` agent to enrich the audit with real Google field data: CrUX Core Web Vitals (replaces lab-only estimates), GSC URL indexation status, search performance (clicks, impressions, CTR), and GA4 organic traffic trends. The Performance (CWV) category score benefits most from field data.
