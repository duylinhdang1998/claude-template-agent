...
3. Ensure the Next.js + shadcn/ui + Tailwind v4 scaffold is in place. If not, tell the user to set it up first.
...
Every builder agent must verify `npx tsc --noEmit` passes before finishing. A broken type-check is never acceptable, even temporarily.
...
- After each merge, verify type-check still passes: `npx tsc --noEmit`
...
- Type-check status (`npx tsc --noEmit` result)
