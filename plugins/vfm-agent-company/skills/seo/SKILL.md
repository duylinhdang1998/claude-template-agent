...
2. Spawn subagents: google-seo-technical-lead, google-seo-content-strategist, google-seo-schema-architect, google-seo-sitemap-architect, google-seo-performance-engineer, google-seo-visual-analyst, google-seo-geo-strategist
3. If Google API credentials detected (`python3 scripts/google_auth.py --check`), also spawn google-search-console-analyst agent
4. If local business detected, also spawn google-seo-local-specialist agent
5. If local business detected AND DataForSEO MCP available, also spawn google-seo-maps-specialist agent
6. If backlink APIs detected (`python3 scripts/backlinks_auth.py --check`), also spawn ahrefs-backlinks-analyst agent
7. If Firecrawl MCP available, use `firecrawl_map` to discover all site URLs before analysis
8. If content strategy signals detected (blog, pillar pages, topic clusters), also spawn google-seo-cluster-strategist agent
9. If e-commerce detected, also spawn google-seo-ecommerce-specialist agent
10. If drift baseline exists for this URL (`python3 scripts/drift_history.py <url>`), also spawn google-seo-drift-monitor agent
11. Always include google-seo-sxo-specialist in full audits (search experience applies to all sites)
...
- `google-seo-technical-lead` -- Crawlability, indexability, security, CWV
- `google-seo-content-strategist` -- E-E-A-T, readability, thin content
- `google-seo-schema-architect` -- Detection, validation, generation
- `google-seo-sitemap-architect` -- Structure, coverage, quality gates
- `google-seo-performance-engineer` -- Core Web Vitals measurement
- `google-seo-visual-analyst` -- Screenshots, mobile testing, above-fold
- `google-seo-geo-strategist` -- AI crawler access, llms.txt, citability, brand mention signals
- `google-seo-local-specialist` -- GBP signals, NAP consistency, reviews, local schema, industry-specific local factors (conditional: spawned when Local Service detected)
- `google-seo-maps-specialist` -- Geo-grid rank tracking, GBP audit, review intelligence, competitor radius mapping (conditional: spawned when Local Service detected AND DataForSEO MCP available)
- `google-search-console-analyst` -- CWV field data, URL indexation status, organic traffic trends (conditional: spawned when Google API credentials detected)
- `ahrefs-backlinks-analyst` -- Backlink profile data: DA/PA, referring domains, anchor text, toxic links (conditional: spawned when Moz/Bing API keys detected or always for CC domain-level metrics)
- `google-seo-cluster-strategist` -- Semantic clustering analysis (conditional: content strategy detected)
- `google-seo-sxo-specialist` -- Page-type mismatch, user stories, persona scoring (always in full audits)
- `google-seo-drift-monitor` -- Baseline comparison (conditional: drift baseline exists for URL)
- `google-seo-ecommerce-specialist` -- Product schema, marketplace intel (conditional: e-commerce detected)
- `google-seo-flow-analyst` -- FLOW framework prompts (conditional: spawned for content strategy workflows)
- `dataforseo-data-analyst` -- Live SERP, keyword, backlink, local SEO data (extension, optional)
- `google-seo-image-strategist` -- SEO image audit and generation plan (extension, optional)
