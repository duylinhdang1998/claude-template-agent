...
- Schema.org markup actually present (via `google-seo-schema-architect`)
- SERP visibility for the site's published topics (via `dataforseo-data-analyst` /
  Google APIs when available)
- Backlink + brand-mention landscape (via `ahrefs-backlinks-analyst`)
- Core Web Vitals field data from CrUX (via `scripts/pagespeed_check.py`)
- AI-search citation patterns (via `google-seo-geo-strategist`)
...
- `google-seo-content-strategist` thin-content finding × `google-seo-cluster-strategist` SERP-overlap data →
  consolidate three weak pages into one cluster hub.
- `google-seo-schema-architect` missing Product schema × `google-seo-ecommerce-specialist` UCP-not-declared
  → both close the same agent-era buying gap; bundle as one
  recommendation.
- `google-seo-geo-strategist` low AI-citation rate × `ahrefs-backlinks-analyst` brand-mention
  underweight → mentions matter 3× more than backlinks for AI
  citations; reframe link-building budget into PR / Reddit / YouTube.
- `google-seo-technical-lead` SPA detection × `google-seo-content-strategist` missing main-content
